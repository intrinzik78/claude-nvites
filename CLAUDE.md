...
- **running `gcloud`, `gsutil`, or `bq` without an explicit project flag** - shell defaults and active configurations can silently drift; only the inline flag (`--project=<id>`, `-p <id>`, `--project_id=<id>`) is authoritative. Every invocation must include one. This rule fires on the command itself, not on project awareness — there is no "obvious default" that lets you skip the flag.